//用D3.js创建了一个交互式的地质时间刻度图，
//提供了丰富的配置选项和交互功能，包括点击缩放、鼠标悬停高亮、自动适应窗口大小等。
//通过这种方式，可以直观地展示和浏览地质时间数据
import { zoom as d3zoom, partition, select, stratify } from "d3";
...
export default class GeoTimeLine {
  /** text font */
  readonly font: string;   //字体样式
  /** svg object */
  readonly svg: Selection<SVGSVGElement, unknown, HTMLElement, any>;  //SVG元素的选择器
  /** interval data */
  readonly intervals: IntervalItem[];  //地质时间间隔数据
  /** hierarchical data generated by intervals */
  readonly hierarchicalData: HierarchyNode<IntervalItem>;  //由地质时间间隔数据生成的分层数据
  /** the root hierarchical data */
  readonly root: NodeItem;  //根节点数据
  /** user input options */
  readonly options: GeoTimeScaleOptions;  //用户输入的选项
  private _onChange: (node: NodeItem) => void;  //节点变化时的回调函数
  private _ready: boolean;  //表示是否已准备好
  private _cellGroup: Selection<SVGGElement, unknown, HTMLElement, any>;  //用于管理和操作SVG元素的选择器
  private _cell: Selection<  //用于管理和操作SVG元素的选择器
    SVGGElement | BaseType,
    NodeItem,
    SVGGElement,
    unknown
  >;
  private _rect: Selection<SVGRectElement, NodeItem, SVGGElement, unknown>;//用于管理和操作SVG元素的选择器
  private _text: Selection<SVGTextElement, NodeItem, SVGGElement, unknown>;//用于管理和操作SVG元素的选择器
  /** get or set animation transition time */
  transition: number;             //动画过渡时间
  private _simplify: boolean;     //各种内部状态变量
  private _focus: NodeItem;       //各种内部状态变量
  private _sequence: NodeItem[];  //各种内部状态变量
  private _neighborWidth: number;//各种内部状态变量
  private _tickLength: number;   //各种内部状态变量
  private _ticksGroup: Selection<SVGGElement, unknown, HTMLElement, any>;//用于管理和操作SVG元素的选择器
  ...
  constructor(
    ...
    this.root = partition<IntervalItem>()
      .size([
        width,
        (height - opts.margin.bottom - opts.margin.top) *
          (simplify ? (this.hierarchicalData.height + 1) / 2 : 1),
      ])
  ...
  set stage(val: string) {
    ...
    if (node) {
      this._clicked(undefined, node, "notChange");
    }
  }
  ...
  private _init() {
    ...
    self._rect = self._drawRect(self._cell).on("click", self._clicked);

    self._addTitle(self._cell);

    // draw text
    self._text = self._drawText(self._cell);
    self._ticksGroup = svg.append("g").attr("id", "ticks");
    ...
    function zoomed(e: d3ZoomEvent) {
      ...
      if (event.type === "wheel") {
        if ((event as any).wheelDelta < 0) {
          ...
          const { offsetX } = event;
          const offsetY =
            event.offsetY + (self._simplify ? self._focus?.y0 ?? 0 : 0);
          const node = self.root.find(
            (node) =>
              node.target.x0 <= offsetX &&
              node.target.x1 > offsetX &&
              node.target.y0 <= offsetY &&
              node.target.y1 > offsetY,
          );
          if (node) {
            self._focus = null;
    ...
    const ticksData = this.root.descendants().map((d) => {
      ...
      if (visible) {
        if (
          !ifFocusBottom &&
          d.parent?.visible &&
          d.data.start === d.parent.data.start
        ) {
          visible = false;
        } else {
      ...
      return {
        ...
        y:
          d.y0 -
          (this._simplify ? (ifFocusBottom ? focus.parent.y0 : focus.y0) : 0),
        depth: d.depth,
  ...
  private _updateTicks<
    T extends Selection<BaseType, TickNode, BaseType, unknown>,
  >(node: T): T {
    return node
  ...
  private _clicked = (
    ...
    const rightNeighbor =
      focus.data.end === this.root.data.end ? 0 : this._neighborWidth;
    const widthMinusNeighbors =
      this.options.width - rightNeighbor - leftNeighbor;
    ...
    this.root.each((d) => {
      ...
      if (this._simplify) {
        d.visible =
          [focus.depth, focus.depth + 1].includes(d.depth) ||
          (!focus.children && d.depth === focus.depth - 1);
      }
    });
    ...
    trans(this._text, duration)
      ...
      .attr("opacity", (d) => (d.visible ? 1 : 0))
      .attr("font-size", (d) =>
        d.id === focus.id ? this.options.fontSize + 6 : this.options.fontSize,
      )
    ...
    if (this._simplify) {
      trans(this._cell, duration).style("opacity", (d) => (d.visible ? 1 : 0));

      trans(this._cellGroup, duration).attr(
        "transform",
        `translate(0, ${
          !this._focus.children
            ? -(this._focus.parent?.y0 ?? 0)
            : -this._focus.target.y0
        })`,
      );
    }
